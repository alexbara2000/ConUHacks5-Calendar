			...
			if (user == 0) {
				...
				if (option_No == 1) {
					...
					for (int c = 0; c < 1;) {
						...
						System.out.println(
								"Please enter the start time, then press enter and the the end date in army format: ");
						int startTime = keys.nextInt();
						tea[tNum].setExam_St(startTime);
						int endTime = keys.nextInt();
						tea[tNum].setExam_End(endTime);

						if ((dayOfWeek >= 1 && dayOfWeek <= 7) && (startTime >= 0 && startTime > 830)
								&& (endTime >= 0 && endTime < 2200)) {

							Exams e1 = new Exams(startTime, endTime, dayOfWeek);
							tea[tNum].setExam(e1);
							System.out.println(tea[tNum].getExam());

						}
						/*
						 * for(int t =0; t<tea.length; t++){ if(t != tNum){ boolean check =
						 * (tea[tNum].getExam_End() >= tea[t].getExam_End() && tea[tNum].getExam_St()<=
						 * tea[t].getExam_End()) || (tea[tNum].getExam_St() >= tea[t].getExam_St() &&
						 * tea[tNum].getExam_End()<= tea[t].getExam_End()) || tea[tNum].getExam_St() <=
						 * tea[t].getExam_St() && tea[tNum].getExam_End()>= tea[t].getExam_End();
						 * if(check == true){ tcount++; } } if(t == tNum){ continue;
						 * 
						 * }
						 * 
						 * } if(tcount ==0) { Exams e1= new Exams(startTime, endTime, dayOfWeek );
						 * forCopy.setExamStart(startTime); forCopy.setExamEnd(endTime);
						 * forCopy.setExamDay(dayOfWeek);
						 * System.out.println("the exam now starts at "+forCopy.getExamStart()+" " +
						 * " and ends at "+ forCopy.getExamEnd() + " on day number "+
						 * forCopy.getExamDay() +" of the week!"); c++; }
						 * 
						 * }
						 * 
						 * else { System.out.println("Sorry that is not a valid day of the week!"); }
						 * if(tcount !=0) {
						...
						for (int c = 0; c < 1;) {
							...
							if ((dayOfWeek >= 1 && dayOfWeek <= 7) && (startTime >= 0 && startTime > 830)
								...
								for (int t = 0; t < tea.length; t++) {
									if (t != tNum) {
										boolean check = (tea[tNum].getExam_End() >= tea[t].getExam_End()
												&& tea[tNum].getExam_St() <= tea[t].getExam_End())
												|| (tea[tNum].getExam_St() >= tea[t].getExam_St()
														&& tea[tNum].getExam_End() <= tea[t].getExam_End())
												|| tea[tNum].getExam_St() <= tea[t].getExam_St()
														&& tea[tNum].getExam_End() >= tea[t].getExam_End();
										if (check == true) {
											tcount++;
								...
								if (tcount == 0) {
									...
									System.out.println("the exam now starts at " + forCopy.getExamStart() + " "
											+ " and ends at " + forCopy.getExamEnd() + " on day number "
											+ forCopy.getExamDay() + " of the week!");
								}
